...
#####  <a href="https://www.codecogs.com/eqnedit.php?latex=\mathbf{min\sum_{i=1}^{K}\sum_{x\in&space;C_i}||x-\mu_i||^2}" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\mathbf{min\sum_{i=1}^{K}\sum_{x\in&space;C_i}||x-\mu_i||^2}" title="\mathbf{min\sum_{i=1}^{K}\sum_{x\in C_i}||x-\mu_i||^2}" /></a>
